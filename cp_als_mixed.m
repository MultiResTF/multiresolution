function [P,Uinit,output, fit] = cp_als_mixed(X,T,R,varargin)
...
% size(X)
% pause
normX = sqrt(aval^2 + norm(T)^2 - 2*innerprod(X,T));
...
if (isa(X,'sptensor') || isa(X,'tensor')) && (exist('cpals_core','file') == 3)
    ...
    for iter = 1:maxiters
        ...
        for n = dimorder(1:end)
            ...
            Unew = mttkrp(X,U,n) - mttkrp(T,U,n);
...
%         fit = 1 - (normresidual / normX); %fraction explained by model
        normshow = aval;%sqrt(normX^2 - norm(T)^2 + 2*innerprod(X,T));

        fit = 1 - (normresidual / normX); %fraction explained by model
        fitshow = 1 - (normresidual / normshow); %fraction explained by model

        fitchange = abs(fitold - fit);
        
        if mod(iter,printitn)==0
%             fprintf(' Iter %2d: fit = %e fitdelta = %7.1e\n', iter, fit, fitchange);
            fprintf(' Iter %2d: fit = %e fitdelta = %7.1e\n', iter, fitshow, fitchange);
        end
        
        % Check for convergence
        if (iter > 1) && (fitchange < fitchangetol)
%            fprintf('%1.16e', (normresidual / normX))
           break;
        end        
    end   
end
