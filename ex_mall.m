...
% print -dpdf mall_storageM200.pdf
...
legend({'Multiscale/iteration', 'Tensor-train'},...
        ...
        'FontName','Times')
    pbaspect([2 1 1])
% print -dpdf mall_timeM200.pdf
%%

%%Visualize the results
figure(2)
clf
B1 = reshape(full(A1), [n,n,n]);
Bdirect = reshape(full(Adirect),[n,n,n]);
for k = 1:n
     subplot(1,3,1)
    imagesc(mat2gray(A_orig(:,:,k)))
    colormap('gray'); brighten(0.5);
    title('Full tensor')
     axis off   
    subplot(1,3,2)
    imagesc(mat2gray(B1(:,:,k)))
    colormap('gray'); brighten(0.5);
    title('Multiscale-representation')
     axis off
    subplot(1,3,3)
    imagesc(mat2gray(Bdirect(:,:,k)))
    colormap('gray'); brighten(0.5);
    title('TT-representation')
     axis off
    pause(0.01)
end

%%
%Visualize the sum of a few different scales.
clf
figure(3)
Anew = res2{levels} + downscale_TT(res2{levels-1})...
     + downscale_TT(downscale_TT(res2{levels-2}))...
     + downscale_TT(downscale_TT(downscale_TT(res2{levels-3})))...
     + downscale_TT(downscale_TT(downscale_TT(downscale_TT(res2{levels-4}))));
Bnew = reshape(full(Anew), [n,n,n]);
for k = 1:n
    imagesc(mat2gray(Bnew(:,:,k)))
     colormap('gray'); brighten(0.5);
     axis off
    pause(0.05)
end
